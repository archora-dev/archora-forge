...
<template>
  <section class="reports-module">
    <header class="reports-module__header">
      <div>
        <p class="reports-module__eyebrow">Report management</p>
        <h2>Reports</h2>
      </div>
    </header>

    <div v-if="props.loading" class="reports-module__state">Loading reports...</div>
    <div v-else-if="props.error" class="reports-module__state reports-module__state--error">
      {{ props.error }}
    </div>
    <div v-else-if="props.rows.length === 0" class="reports-module__state">No reports found</div>

    <ArchDataTable v-else :rows="props.rows" :columns="tableColumns" class="reports-table">
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="(row, rowIndex) in props.rows" :key="row.id ? String(row.id) : rowIndex">
            <td v-for="column in tableColumns" :key="column.name">
              ...
              <span v-else :class="['reports-table__cell', `reports-table__cell--${column.cell}`]">
                {{ formatCellValue(row, column) }}
              </span>
            </td>
            <td v-if="hasRowActions" class="reports-table__actions"></td>
          </tr>
        </tbody>
      </table>
    </ArchDataTable>

    <footer v-if="pagination.enabled" class="reports-module__pagination">
...
.reports-module__header h2 {
  margin: 0;
  color: #f8fafc;
  font-size: 22px;
}

.reports-module__primary,
.reports-table__actions button {
...
.reports-module__primary:hover,
.reports-table__actions button:hover {
  border-color: rgba(125, 211, 252, 0.55);
  background: rgba(14, 165, 233, 0.14);
  color: #f8fafc;
}

.reports-module__state {
...
.reports-table__cell--number,
.reports-table__cell--date,
.reports-table__cell--dateTime {
  color: #b8c7d9;
}

.reports-table__cell--json {
  color: #d6e4f0;
}

.reports-table :deep(.arch-badge) {
  display: inline-flex;
  align-items: center;
  min-height: 24px;
